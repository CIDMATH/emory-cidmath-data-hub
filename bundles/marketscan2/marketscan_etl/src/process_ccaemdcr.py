"""
Process CCAEMDCR files to Delta tables.

Pattern: /Volumes/.../CCAEMDCR/2014 v30/Set A/PARQUET/ccaea143_a_0_0_0.snappy.parquet
Table:   raw_marketscan_set_a_ccaea_2014_v30

Grouping: First 5 chars of filename + year/version + set
"""
...
def derive_table_name(file_path: str) -> str:
    """
    Extract table name from CCAEMDCR file path.
    
    Example:
        /CCAEMDCR/2014 v30/Set A/PARQUET/ccaea143_a_0_0_0.snappy.parquet
        → raw_marketscan_set_a_ccaea_2014_v30
    """
    ...
    set_id = None
    for part in parts:
        match = re.match(r'^Set\s+([AB])$', part, re.IGNORECASE)
        if match:
            set_id = match.group(1).lower()
            break
    
    if not year or not version or not set_id:
        raise ValueError(f"Missing components in path: {file_path}")
    
    table_name = f"raw_marketscan_set_{set_id}_{table_code}_{year}_{version}"
    return table_name.lower()


def get_file_prefix(file_path: str) -> str:
    """
    Create grouping key: table_code|year_version|set
    
    Example: ccaea|2014_v30|set_a
    """
    parts = file_path.split("/")
    ...
    for part in parts:
        match = re.match(r'^Set\s+([AB])$', part, re.IGNORECASE)
        if match:
            set_id = f"set_{match.group(1).lower()}"
            break
